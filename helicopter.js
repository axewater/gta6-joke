...
import { policeBulletPool, missilePool } from './object-pool.js';
...
export function updateHelicopter(dt) {
  ...
  if (state.wantedLevel < 3 && state.helicopter) {
    ...
    // Clean up missiles from flat array
    const missiles = state.heliMissiles;
    for (let i = missiles.length - 1; i >= 0; i--) {
      missilePool.release(missiles[i].mesh);
    }
    state.heliMissiles = [];
  ...
  // Missile updates handled by missileSystem in systems.js
}
...
function fireMissile(heli, playerX, playerZ) {
  const mesh = missilePool.acquire();
  if (!mesh) return;
